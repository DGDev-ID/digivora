...
<template>
  ...
  >
    <canvas
      ref="canvasRef"
      class="absolute inset-0 w-full h-full pointer-events-none opacity-[0.05]"
    />
    ...
    <!-- EXTREME: System Core Illustration (Center background) — tetap tampil di semua ukuran -->
    <div
      class="system-core absolute left-1/2 top-[40%] -translate-x-1/2 -translate-y-1/2 w-[900px] h-[900px] pointer-events-none select-none z-0 opacity-[0.06]"
    >
      <svg viewBox="0 0 900 900" fill="none" class="w-full h-full">
        ...
        <g class="system-core-outer">
          ...
          />
          <path
            d="M 450 10 L 450 50 M 450 850 L 450 890 M 10 450 L 50 450 M 850 450 L 890 450"
            stroke="#26C6B7"
            stroke-width="4"
          />
          <!-- Outer nodes -->
    ...
    <div
      class="proc-big-num absolute right-0 bottom-0 pointer-events-none select-none z-0"
      style="
    ...
    <!-- Ambient glow -->
    <div
      class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] pointer-events-none z-0"
      style="background: radial-gradient(circle, rgba(0, 191, 166, 0.08) 0%, transparent 65%)"
    ></div>

    <!-- Float 1: Large hexagon (top-left) — hanya desktop -->
    <div
      class="proc-float-1 hidden lg:block absolute left-[2%] top-[5%] pointer-events-none select-none opacity-[0.08] z-0"
    >
      <svg width="220" height="220" viewBox="0 0 220 220" fill="none">
        ...
        />
        <polygon
          points="110,40 175,78 175,142 110,180 45,142 45,78"
          stroke="#00BFA6"
          stroke-width="0.7"
        />
        <polygon
          points="110,70 150,93 150,127 110,150 70,127 70,93"
          stroke="#00BFA6"
          stroke-width="0.4"
        />
    ...
    <!-- Float 2: Circuit cross (top-right) — hanya desktop -->
    <div
      class="proc-float-2 hidden lg:block absolute right-[5%] top-[8%] pointer-events-none select-none opacity-[0.08] z-0"
    >
    ...
    <!-- Float 3: Dot grid (right, lower) — hanya desktop -->
    <div
      class="proc-float-3 hidden lg:block absolute right-[3%] bottom-[15%] pointer-events-none select-none opacity-[0.06] z-0"
    >
    ...
    <!-- Float 4: Diamond (left, lower) — hanya desktop -->
    <div
      class="proc-float-4 hidden lg:block absolute left-[5%] bottom-[10%] pointer-events-none select-none opacity-[0.08] z-0"
    >
    ...
    <!-- Float 5: Triangle top-center — hanya desktop -->
    <div
      class="proc-float-5 hidden lg:block absolute left-[45%] top-[2%] pointer-events-none select-none opacity-[0.08] z-0"
    >
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-6">
      ...
      <div class="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-20">
        <div>
          <!-- Char label -->
          <div
            class="proc-label-chars flex items-center gap-4 mb-6 overflow-hidden"
            style="perspective: 500px"
          >
            <div
              class="h-px w-8 flex-shrink-0"
              style="background-color: rgba(0, 191, 166, 0.5); box-shadow: 0 0 10px #00bfa6"
            ></div>
            <div class="flex">
              <span
          ...
          <div class="process-heading-wrap overflow-hidden pb-2">
            <h2
              class="text-[clamp(2.8rem,5.5vw,5rem)] font-black leading-[1.1] tracking-tight flex flex-wrap gap-x-[0.3em]"
              style="color: #e5e7eb"
            >
              <span
                v-for="w in ['Engineering', 'Process']"
                :key="w"
      ...
      <div class="process-steps hidden lg:block relative z-20">
        <div class="relative mb-8">
          ...
          <div
            ref="connectorRef"
            class="process-connector absolute top-1/2 left-0 right-0 h-[4px] -translate-y-1/2 origin-left"
            style="
              background: linear-gradient(to right, #00bfa6, #26c6b7, rgba(0, 191, 166, 0.8));
          ...
          <div class="flex justify-between relative z-10">
            ...
            >
              <div
                class="process-hex-node relative flex items-center justify-center cursor-default group"
                style="transform: scale(0); opacity: 0"
              >
                <!-- EXTREME: Sonar ping rings -->
                <div
                  class="hex-sonar absolute w-24 h-24 rounded-full border border-[#00BFA6] pointer-events-none"
                ></div>

                <!-- Pulse glow -->
                <div
                  class="hex-glow absolute w-24 h-24 rounded-full pointer-events-none"
                  style="
                    background: radial-gradient(circle, rgba(0, 191, 166, 0.4), transparent 70%);
                    opacity: 0;
                    transform: scale(1);
                  "
                ></div>
                ...
                <svg
                  ...
                  viewBox="0 0 48 54"
                  fill="none"
                  class="transition-transform duration-500 group-hover:scale-110 drop-shadow-[0_0_15px_rgba(0,191,166,0.3)]"
                >
                  <path
                    d="M24 2 L45 14 L45 40 L24 52 L3 40 L3 14 Z"
                    stroke="#00BFA6"
                    stroke-width="2"
                    fill="#0B1312"
                  />
                ...
                <span
                  class="absolute text-[18px] font-black font-mono transition-colors duration-300 group-hover:text-white"
                  style="color: #00bfa6; text-shadow: 0 0 10px rgba(0, 191, 166, 0.5)"
                  >{{ step.num }}</span
                >
              </div>
            </div>
        ...
        <div class="process-steps-content flex justify-between relative z-20">
          ...
          >
            <div
              class="text-[11px] font-mono tracking-[0.3em] font-bold mb-3 px-3 py-1 rounded bg-[#00BFA6]/10 border border-[#00BFA6]/20"
              style="color: rgba(0, 191, 166, 0.7)"
            >
              {{ step.tag }}
            </div>
            <h3
              class="text-2xl font-bold tracking-wide group-hover:text-[#00BFA6] transition-colors duration-300 drop-shadow-md"
              style="color: #e5e7eb"
            >
              {{ step.title }}
            </h3>
      ...
      <div
        class="process-steps-mobile lg:hidden flex flex-col gap-0 relative z-20"
        style="border-top: 1px solid rgba(0, 191, 166, 0.2)"
      >
        <div
          v-for="step in steps"
          :key="step.num"
        ...
        >
          <div class="shrink-0 relative flex items-center justify-center">
            <div
              class="hex-sonar absolute w-16 h-16 rounded-full border border-[#00BFA6] pointer-events-none"
            ></div>
            ...
            <span
              class="absolute text-[14px] font-black font-mono"
              style="color: #00bfa6; text-shadow: 0 0 5px rgba(0, 191, 166, 0.5)"
              >{{ step.num }}</span
            >
          </div>
          <div>
            <div
              class="text-[10px] font-mono tracking-widest mb-2 font-bold px-2 py-0.5 rounded bg-[#00BFA6]/10 inline-block border border-[#00BFA6]/20"
              style="color: rgba(0, 191, 166, 0.6)"
            >
              {{ step.tag }}
            </div>
            <h3
              class="text-xl font-bold group-hover:text-[#00BFA6] transition-colors duration-300"
              style="color: #e5e7eb"
            >
              {{ step.title }}
            </h3>

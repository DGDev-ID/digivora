...
<template>
  <section id="cta" ref="sectionRef" class="relative py-48 md:py-64 overflow-hidden" style="background-color: #0B1312;">
    ...
    <canvas ref="canvasRef" class="absolute inset-0 w-full h-full opacity-25 pointer-events-none z-0"></canvas>

    <!-- Radial glow -->
    <div class="absolute inset-0 pointer-events-none z-0"
      style="background: radial-gradient(ellipse at center, rgba(0,191,166,0.1) 0%, transparent 70%);"></div>

    <!-- ════════════════════════════════════
         EXTREME VISUAL OBJECTS
    ════════════════════════════════════ -->

    <!-- Ghost big text (Glitching) -->
    <div class="cta-ghost-text absolute inset-x-0 top-1/2 -translate-y-1/2 text-center pointer-events-none select-none overflow-hidden z-0"
      style="font-size: clamp(100px, 20vw, 240px); font-weight: 900; color: rgba(0,191,166,1); line-height: 1; letter-spacing: -0.04em; white-space: nowrap; filter: drop-shadow(0 0 30px rgba(0,191,166,0.5)); opacity: 0.04;">
      DIGIVORA
    </div>

    <!-- Float 1: Large ring (top-left) -->
    <div class="cta-float-1 absolute left-[3%] top-[5%] pointer-events-none select-none opacity-[0.09] z-0">
      <svg width="220" height="220" viewBox="0 0 220 220" fill="none">
        <circle cx="110" cy="110" r="100" stroke="#00BFA6" stroke-width="1.5" stroke-dasharray="8 16"/>
        <circle cx="110" cy="110" r="72" stroke="#00BFA6" stroke-width="0.8" stroke-dasharray="4 8"/>
        ...
        <line x1="110" y1="10" x2="110" y2="210" stroke="#00BFA6" stroke-width="0.5" stroke-dasharray="2 10"/>
        <line x1="10" y1="110" x2="210" y2="110" stroke="#00BFA6" stroke-width="0.5" stroke-dasharray="2 10"/>
      </svg>
    </div>

    <!-- Float 2: Hexagon (right, top) -->
    <div class="cta-float-2 absolute right-[4%] top-[8%] pointer-events-none select-none opacity-[0.08] z-0">
      <svg width="180" height="180" viewBox="0 0 180 180" fill="none">
        <polygon points="90,8 168,50 168,130 90,172 12,130 12,50" stroke="#00BFA6" stroke-width="1.5"/>
        <polygon points="90,35 145,67 145,113 90,145 35,113 35,67" stroke="#00BFA6" stroke-width="0.8"/>
    ...
    <div class="cta-float-3 absolute left-[8%] bottom-[8%] pointer-events-none select-none opacity-[0.09] z-0">
    ...
    <div class="cta-float-4 absolute right-[6%] bottom-[10%] pointer-events-none select-none opacity-[0.09] z-0">
    ...
    <div class="cta-float-5 absolute left-[1%] top-[40%] pointer-events-none select-none opacity-[0.06] z-0">
    ...
    <div class="cta-float-6 absolute right-[2%] top-[42%] pointer-events-none select-none opacity-[0.08] z-0">
      <svg width="90" height="78" viewBox="0 0 90 78" fill="none">
        <polygon points="45,4 86,74 4,74" stroke="#00BFA6" stroke-width="1.5" stroke-dasharray="4 6"/>
    ...
    <div class="absolute top-[15%] left-1/2 -translate-x-1/2 z-10 pointer-events-none">
      <svg class="cta-bounce-arrow opacity-30" width="40" height="80" viewBox="0 0 40 80" fill="none">
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-6">
      ...
      <div class="flex items-center gap-4 mb-16 justify-center overflow-hidden" style="perspective: 500px;">
        <div class="h-px w-8 flex-shrink-0" style="background-color: rgba(0,191,166,0.6); box-shadow: 0 0 10px #00BFA6;"></div>
        <div class="flex">
      ...
      <div class="text-center">
        ...
        <div class="flex flex-wrap justify-center gap-x-6 overflow-hidden">
          <div v-for="word in line2Words" :key="'l2-'+word" class="overflow-hidden">
            <span class="cta-word inline-block text-[clamp(3rem,8vw,8rem)] font-black leading-[0.92] tracking-tight drop-shadow-[0_0_20px_rgba(0,191,166,0.4)]"
              style="color: rgba(0,191,166,0.8); display: inline-block;">{{ word }}</span>
          </div>
        </div>
        <!-- Line 3 -->
        ...
        <!-- Actions -->
        <div class="cta-actions flex flex-col sm:flex-row items-center justify-center gap-6" style="opacity: 0;">
          <RouterLink to="/contact"
            ...
            style="background-color: #00BFA6; color: #0B1312; box-shadow: 0 10px 30px rgba(0,191,166,0.3);">
            <Mail :size="16" />
            Start a Project

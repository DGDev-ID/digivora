...
<template>
  <section id="services" ref="sectionRef" class="relative py-36 md:py-56 overflow-hidden"
    ...
    <!-- EXTREME: Giant Outline Marquee -->
    <div class="absolute top-1/2 -translate-y-1/2 left-0 w-full overflow-hidden pointer-events-none select-none z-0 opacity-10">
      <div class="svc-marquee-track flex whitespace-nowrap" style="width: 200%;">
        <h1 class="text-[clamp(150px,25vw,350px)] font-black uppercase"
    ...
    <!-- Ghost big number -->
    <div class="svc-big-num absolute left-0 bottom-8 pointer-events-none select-none z-0"
    ...
    <!-- Neon tracker line -->
    <div class="absolute left-0 top-0 w-[3px] h-full overflow-hidden pointer-events-none" style="z-index:5;">
      <div ref="trackerRef" class="absolute left-0 top-0 w-full"
        style="height: 200px; background: linear-gradient(to bottom, transparent, #00BFA6, #26C6B7, #fff, transparent); filter: blur(1.5px); box-shadow: 0 0 20px rgba(0,191,166,1); top: -200px;"></div>
    </div>

    <!-- Float 1: Circle rings (top right) -->
    <div class="svc-float-1 absolute right-[4%] top-[5%] pointer-events-none select-none opacity-[0.08] z-0">
      <svg width="200" height="200" viewBox="0 0 200 200" fill="none">
        <circle cx="100" cy="100" r="90" stroke="#00BFA6" stroke-width="1.2" stroke-dasharray="6 12"/>
        <circle cx="100" cy="100" r="65" stroke="#00BFA6" stroke-width="0.8" stroke-dasharray="4 8"/>
        ...
        <line x1="100" y1="10" x2="100" y2="190" stroke="#00BFA6" stroke-width="0.5" stroke-dasharray="2 8"/>
        <line x1="10" y1="100" x2="190" y2="100" stroke="#00BFA6" stroke-width="0.5" stroke-dasharray="2 8"/>
      </svg>
    </div>

    <!-- Float 2: Hexagon (left, mid) -->
    <div class="svc-float-2 absolute left-[1%] top-[40%] pointer-events-none select-none opacity-[0.07] z-0">
      <svg width="160" height="160" viewBox="0 0 160 160" fill="none">
        <polygon points="80,8 148,44 148,116 80,152 12,116 12,44" stroke="#00BFA6" stroke-width="1.5"/>
        <polygon points="80,28 128,56 128,104 80,132 32,104 32,56" stroke="#00BFA6" stroke-width="1"/>
    ...
    <div class="svc-float-3 absolute right-[2%] bottom-[10%] pointer-events-none select-none opacity-[0.06] z-0">
      <svg width="150" height="150" viewBox="0 0 150 150" fill="none">
        <g fill="#00BFA6">
          <circle cx="15" cy="15" r="2.5"/><circle cx="45" cy="15" r="2.5"/><circle cx="75" cy="15" r="2.5"/><circle cx="105" cy="15" r="2.5"/><circle cx="135" cy="15" r="2.5"/>
          <circle cx="15" cy="45" r="2.5"/><circle cx="45" cy="45" r="2.5"/><circle cx="75" cy="45" r="2.5"/><circle cx="105" cy="45" r="2.5"/><circle cx="135" cy="45" r="2.5"/>
          <circle cx="15" cy="75" r="2.5"/><circle cx="45" cy="75" r="2.5"/><circle cx="75" cy="75" r="2.5"/><circle cx="105" cy="75" r="2.5"/><circle cx="135" cy="75" r="2.5"/>
          <circle cx="15" cy="105" r="2.5"/><circle cx="45" cy="105" r="2.5"/><circle cx="75" cy="105" r="2.5"/><circle cx="105" cy="105" r="2.5"/><circle cx="135" cy="105" r="2.5"/>
          <circle cx="15" cy="135" r="2.5"/><circle cx="45" cy="135" r="2.5"/><circle cx="75" cy="135" r="2.5"/><circle cx="105" cy="135" r="2.5"/><circle cx="135" cy="135" r="2.5"/>
        </g>
      </svg>
    </div>

    <!-- Float 4: Triangle (top-left area) -->
    <div class="svc-float-4 absolute left-[20%] top-[3%] pointer-events-none select-none opacity-[0.08] z-0">
    ...
    <div class="max-w-7xl mx-auto px-6 relative z-10">
      ...
      <div class="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16 relative z-10">
        <div>
          ...
          <div class="svc-label-chars flex items-center gap-4 mb-6 overflow-hidden" style="perspective: 500px;">
            <div class="h-px w-8 flex-shrink-0" style="background-color: #00BFA6; box-shadow: 0 0 10px #00BFA6;"></div>
            <div class="flex">
          ...
          <!-- Divider -->
          <div class="svc-divider mt-6 h-px w-full origin-left" style="background: linear-gradient(to right, #00BFA6, transparent); transform: scaleX(0); box-shadow: 0 0 15px rgba(0,191,166,0.4);"></div>
        </div>

        <div class="flex flex-col gap-4 md:items-end">
          <RouterLink to="/services" class="inline-flex items-center gap-2 text-xs font-bold tracking-[0.2em] uppercase transition-all duration-300 hover:gap-4 px-6 py-3 rounded-full" style="color: #00BFA6; border: 1px solid rgba(0,191,166,0.3); background: rgba(0,191,166,0.05); backdrop-filter: blur(5px);">
      ...
      <div class="svc-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 relative z-10">
        <RouterLink v-for="(svc, i) in services" :key="svc.num"
          ...
          style="min-height: 280px; background-color: #11201F; opacity: 0; box-shadow: 0 10px 40px rgba(0,0,0,0.5);"
        >
          <!-- Image bg -->
          <div class="absolute inset-0 z-0 overflow-hidden">
            <img :src="svc.img" :alt="svc.title"
              ...
              loading="lazy"
            />
            <div class="absolute inset-0 transition-all duration-700" style="background: rgba(11,19,18,0.92);" :class="'group-hover:opacity-0'"></div>
            <div class="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700"
              style="background: linear-gradient(to top, rgba(11,19,18,0.98) 0%, rgba(11,19,18,0.4) 100%);"></div>
          </div>

          <!-- Scroll-glow -->
          <div class="svc-card-glow absolute inset-0 rounded-2xl transition-opacity duration-500 pointer-events-none z-10"
            style="box-shadow: inset 0 0 0 2px rgba(0,191,166,0.8), 0 0 60px rgba(0,191,166,0.2); opacity: 0;"></div>
          
          <!-- Hover border -->
          <div class="absolute inset-0 rounded-2xl pointer-events-none z-10" style="border: 1px solid rgba(0,191,166,0.2);"></div>
          <div class="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-400 pointer-events-none z-10"
            style="box-shadow: inset 0 0 0 1px rgba(0,191,166,0.8), 0 0 30px rgba(0,191,166,0.3);"></div>

          <!-- Content (Minimalist) -->
          <div class="relative z-20 p-8 w-full flex flex-col items-center">
            <div class="w-16 h-16 rounded-2xl flex items-center justify-center transition-all duration-500 group-hover:scale-110 mb-6"
              style="background: rgba(0,191,166,0.15); border: 1px solid rgba(0,191,166,0.3); backdrop-filter: blur(5px); box-shadow: 0 0 20px rgba(0,191,166,0.2);">
              <component :is="svc.icon" :size="28" style="color: #00BFA6; filter: drop-shadow(0 0 5px #00BFA6);" />
            </div>

            <h3 class="text-xl font-bold mb-4 tracking-wide transition-colors duration-500 group-hover:text-[#26C6B7] drop-shadow-lg" style="color: #E5E7EB;">{{ svc.title }}</h3>
            ...
            <div class="flex flex-wrap justify-center gap-2 mb-6">
              <span v-for="tag in svc.tags" :key="tag"
                ...
                style="border: 1px solid rgba(0,191,166,0.3); color: rgba(0,191,166,0.7); background: rgba(0,0,0,0.4);"
              >{{ tag }}</span>
            </div>

            <div class="flex items-center justify-center gap-2">
              <span class="text-[10px] font-bold tracking-[0.25em] uppercase transition-colors duration-500 group-hover:text-[#00BFA6] group-hover:text-shadow-[0_0_8px_#00BFA6]" style="color: rgba(229,231,235,0.4);">Explore Module</span>
              <ArrowUpRight :size="14" class="opacity-0 group-hover:opacity-100 transition-all duration-500 -translate-x-3 group-hover:translate-x-0 text-[#00BFA6]" />
            </div>
          </div>
        </RouterLink>
      </div>

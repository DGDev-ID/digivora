  ...
  ctx = gsap.context(() => {
    ...
    gsap.fromTo('.about-quote',
      { clipPath: 'inset(0 100% 0 0)', opacity: 0 },
      {
        ...
        scrollTrigger: { trigger: '.about-quote', start: 'top 85%', toggleActions: 'play reverse play reverse' },
      },
    )

    gsap.fromTo('.about-action',
      { opacity: 0, y: 20 },
      {
...
<template>
  ...
  >
    ...
    <!-- Big ghost number -->
    <div class="about-big-num absolute right-0 top-16 pointer-events-none select-none z-0"
    ...
    <div class="about-torus absolute -right-[20%] top-[10%] w-[800px] h-[800px] pointer-events-none select-none z-0 opacity-[0.04]">
      <svg viewBox="0 0 800 800" fill="none" class="w-full h-full">
        <g class="about-torus-outer">
          ...
          <ellipse cx="400" cy="400" rx="380" ry="150" stroke="#00BFA6" stroke-width="1.5" transform="rotate(45 400 400)"/>
          <ellipse cx="400" cy="400" rx="380" ry="150" stroke="#00BFA6" stroke-width="1.5" transform="rotate(90 400 400)"/>
          <ellipse cx="400" cy="400" rx="380" ry="150" stroke="#00BFA6" stroke-width="1.5" transform="rotate(135 400 400)"/>
        </g>
        <g class="about-torus-inner">
          <circle cx="400" cy="400" r="230" stroke="#26C6B7" stroke-width="1" stroke-dasharray="10 20"/>
          <ellipse cx="400" cy="400" rx="230" ry="90" stroke="#26C6B7" stroke-width="1"/>
          <ellipse cx="400" cy="400" rx="230" ry="90" stroke="#26C6B7" stroke-width="1" transform="rotate(60 400 400)"/>
          <ellipse cx="400" cy="400" rx="230" ry="90" stroke="#26C6B7" stroke-width="1" transform="rotate(120 400 400)"/>
        </g>
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-6">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-start relative z-10">
        ...
        <div>
          ...
          <div
            ...
            style="background: linear-gradient(to right, #00BFA6, rgba(0,191,166,0)); transform: scaleX(0); box-shadow: 0 0 15px rgba(0,191,166,0.5);"
          ></div>
          ...
          <div class="mt-8 flex flex-wrap gap-2">
            <span
              ...
              style="border: 1px solid rgba(0,191,166,0.3); color: rgba(0,191,166,0.7); background: rgba(0,191,166,0.05);"
            >{{ tech }}</span>
          </div>
        </div>
        ...
        <div class="flex flex-col gap-10 pt-4">
          <p class="about-quote text-2xl md:text-3xl leading-relaxed font-light" style="color: rgba(229,231,235,0.85); clip-path: inset(0 100% 0 0);">
            We engineer <span style="color: #00BFA6; font-weight: 600; text-shadow: 0 0 20px rgba(0,191,166,0.4);">digital infrastructure</span> that becomes the backbone of your business growth.
          </p>

          <div class="about-action">
            <RouterLink to="/about" class="group inline-flex items-center gap-3 px-8 py-4 text-xs font-semibold tracking-[0.2em] uppercase transition-all duration-300 hover:gap-4 hover:bg-[rgba(0,191,166,0.15)] hover:shadow-[0_0_20px_rgba(0,191,166,0.2)]" style="background-color: rgba(0,191,166,0.1); border: 1px solid rgba(0,191,166,0.4); color: #00BFA6;">
              Read Our Story
      ...
      <div
        class="about-values-grid mt-28 grid grid-cols-2 md:grid-cols-4 gap-0 relative z-10"
      >
        <div
          v-for="val in values"
          :key="val.num"
        ...
        >
          ...
          <div class="absolute inset-0 opacity-10 group-hover:opacity-30 transition-opacity duration-500 pointer-events-none flex items-center justify-center">
             <svg width="100%" height="100%" viewBox="0 0 100 100" preserveAspectRatio="none">
               <polygon points="50,5 95,25 95,75 50,95 5,75 5,25" fill="none" stroke="#00BFA6" stroke-width="0.5"/>
             </svg>
          </div>

          <!-- Center glow on hover -->
          <div class="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
            style="background: radial-gradient(circle at center, rgba(0,191,166,0.15), transparent 70%);"></div>

          <div class="text-[10px] font-semibold tracking-[0.35em] uppercase mb-6 relative z-10" style="color: rgba(0,191,166,0.5);">{{ val.num }}</div>
          <div class="text-5xl lg:text-6xl mb-6 font-mono transition-transform duration-500 group-hover:scale-125 group-hover:rotate-[15deg] relative z-10" style="color: rgba(0,191,166,0.4); text-shadow: 0 0 20px rgba(0,191,166,0.2);">{{ val.icon }}</div>
          <h3 class="text-xs lg:text-sm font-bold uppercase tracking-widest group-hover:text-[#00BFA6] transition-colors duration-300 relative z-10" style="color: #E5E7EB;">{{ val.label }}</h3>
        </div>
      </div>
    </div>
  </section>
